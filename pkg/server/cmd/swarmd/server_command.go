...
func parseServerConfig(args []string, stderr io.Writer) (serverConfig, error) {
	...
	fs.StringVar(&cfg.openAIAPIKey, "api-key", "", "OpenAI API key for worker agents (default: OPENAI_API_KEY)")
	fs.StringVar(&cfg.anthropicAPIKey, "anthropic-api-key", "", "Anthropic API key for worker agents (default: ANTHROPIC_API_KEY)")
	fs.StringVar(&cfg.slackUserToken, "slack-user-token", "", "Slack user token for Slack-backed sandbox commands (default: SLACK_USER_TOKEN)")
	...
	cfg.openAIAPIKey = firstNonEmpty(strings.TrimSpace(cfg.openAIAPIKey), strings.TrimSpace(os.Getenv("OPENAI_API_KEY")))
	cfg.anthropicAPIKey = firstNonEmpty(strings.TrimSpace(cfg.anthropicAPIKey), strings.TrimSpace(os.Getenv("ANTHROPIC_API_KEY")))
	cfg.slackUserToken = firstNonEmpty(strings.TrimSpace(cfg.slackUserToken), strings.TrimSpace(os.Getenv("SLACK_USER_TOKEN")))
	if cfg.dataDir == "" {
